...
To complicate matters further, the full BSP is only provided to hardware integrators via closed Rockchip's Gerrit repo, and not to end users. Parts of it contain code which puts limits on distribution.
...
* Any distribution which ships a recent enough kernel version should work out of the box on Flipper One
